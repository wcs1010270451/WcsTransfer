...
export default function PortalKeysPage() {
  ...
  const handleLogReset = async () => {
    ...
    logFilterForm.setFieldsValue(nextValues);
    await loadLogs(1, logPagination.pageSize);
  };
  ...
  const logColumns = [
    ...
    {
      title: "模型",
      dataIndex: "model_public_name",
      key: "model_public_name",
      width: 150,
      render: (value) => value || "-",
    },
    {
  ...
  return (
    <Space direction="vertical" size={24} style={{ width: "100%", padding: 24 }}>
      <PageHeaderCard
        ...
        description="这里可以管理当前工作区的客户端密钥，并查看调用量、Token 消耗、可用模型和调用日志。"
        actions={
          <Space>
        ...
        <Alert
          ...
          description={
            tenant?.status === "disabled"
              ...
              : "当前工作区已注册成功，但还需要管理员激活并设置可创建密钥上限。"
          }
        />
      ) : null}
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} sm={12} lg={6}>
          <Card>
            <Statistic title="请求总数" value={stats?.request_count || 0} formatter={(value) => formatNumber(value)} />
          </Card>
        </Col>
        <Col xs={24} sm={12} lg={6}>
          <Card>
            <Statistic title="总 Token" value={stats?.total_tokens || 0} formatter={(value) => formatNumber(value)} />
          </Card>
        </Col>
        <Col xs={24} sm={12} lg={6}>
          <Card>
            <Statistic title="成功率" value={Number(stats?.success_rate || 0).toFixed(2)} suffix="%" />
          </Card>
        </Col>
        <Col xs={24} sm={12} lg={6}>
          <Card>
            <Statistic title="计费收入" value={stats?.billable_amount || 0} formatter={(value) => formatCurrency(value)} />
          </Card>
        </Col>
      </Row>
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} sm={12} lg={6}>
          <Card>
            <Statistic title="输入 Token" value={stats?.prompt_tokens || 0} formatter={(value) => formatNumber(value)} />
          </Card>
        </Col>
        <Col xs={24} sm={12} lg={6}>
          <Card>
            <Statistic title="输出 Token" value={stats?.completion_tokens || 0} formatter={(value) => formatNumber(value)} />
          </Card>
        </Col>
        <Col xs={24} sm={12} lg={6}>
          <Card>
            <Statistic title="平均延迟" value={Number(stats?.average_latency_ms || 0).toFixed(0)} suffix="ms" />
          </Card>
        </Col>
        <Col xs={24} sm={12} lg={6}>
          <Card>
            <Statistic title="可用密钥数" value={stats?.active_client_keys || 0} formatter={(value) => formatNumber(value)} />
          </Card>
        </Col>
      </Row>
        ...
        <Space direction="vertical" size={18} style={{ width: "100%" }}>
          <Space style={{ width: "100%", justifyContent: "space-between" }} wrap>
            ...
            <Space>
              ...
              <Button type="primary" onClick={handleLogSearch}>
                查询
              </Button>
            </Space>
          </Space>
      ...
      {tenant ? (
        <section className="panel-card">
          ...
          <Space wrap>
            ...
            <Tag color={(tenant.wallet_balance || 0) > 0 ? "green" : "red"}>
              钱包余额：{formatCurrency(tenant.wallet_balance || 0)}
            </Tag>
          </Space>
          {tenant.notes ? (
